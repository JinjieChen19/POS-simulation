...
stan_file_path <- "stan_universal_model.stan"
...
if (file.exists(stan_file_path)) {
  ...
  tryCatch({
    ...
    cat("Please check the stan_universal_model.stan file and try again.\n")
    stop("Stan model compilation failed")
  })
  
} else {
  cat("✗ ERROR: Stan model file not found:", stan_file_path, "\n")
  cat("Please ensure stan_universal_model.stan is in the working directory.\n")
