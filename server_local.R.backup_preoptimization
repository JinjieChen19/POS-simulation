...
function(input, output, session) {
  ...
  observeEvent(input$run_model, {
    ...
    withProgress(message = 'Running Bayesian Analysis', value = 0, {
      ...
      output$model_status <- renderPrint({
        ...
        cat("Using pre-compiled universal model\n")
        cat("(No compilation needed - priors passed as data)\n\n")
        cat("Please wait...\n")
      })
  ...
  output$pos_output <- renderPrint({
    ...
    # Calculate PoS: Pr(HR < 1) = Pr(log(HR) < 0)
    pos_os <- mean(results$posterior_samples$theta_os_post < 0)
    pos_pfs <- mean(results$posterior_samples$theta_pfs_post < 0)
    
    cat("Probability of Success (PoS)\n")
    cat(strrep("=", 60), "\n\n")
    cat("Based on", nrow(results$posterior_samples), "posterior samples\n\n")
    
    cat("Overall Survival (OS):\n")
    cat("  PoS = Pr(HR_OS < 1) = ", sprintf("%.1f%%", pos_os * 100), "\n")
    ...
    cat("  PoS = Pr(HR_PFS < 1) = ", sprintf("%.1f%%", pos_pfs * 100), "\n")
    ...
    cat("Interpretation:\n")
    if (pos_os >= 0.80) {
      cat("  High PoS for OS (≥80%) - Strong evidence of benefit\n")
    } else if (pos_os >= 0.50) {
      cat("  Moderate PoS for OS (50-80%) - Some evidence of benefit\n")
    } else {
      cat("  Low PoS for OS (<50%) - Weak evidence of benefit\n")
    }
  })
